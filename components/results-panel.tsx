...
export function FloatingResultsPanel({ results, loading, error }: FloatingResultsPanelProps) {
  ...
  const copyToClipboard = () => {
    ...
    const data = results.rows.map((row: any) => results.columns.map((col: string) => row[col]).join("\t")).join("\n")
    const text = `${headers}\n${data}`
          ...
          {loading ? (
            ...
            >
              <div className="text-center max-w-md">
                ...
                <h3 className="text-lg font-semibold text-foreground mb-2">Query Error</h3>
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4">
                  <p className="font-mono text-sm text-red-700 dark:text-red-300">{error}</p>
                </div>
              </div>
            </motion.div>
          ) : !results ? (
              ...
              {results.message && (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`font-semibold text-base ${results.error ? "text-red-800" : "text-green-800"}`}>
                      {results.error ? "Error" : "Success"}
                    </span>
                  </div>
                  <div className={`font-mono text-sm ${results.error ? "text-red-700" : "text-green-700"}`}>
                    {typeof results.message === "string"
                      ? results.message.split(/\.\s+/).map((line: string, idx: number, arr: string[]) => (
                          <span key={idx}>
                            {line}
                            {idx < arr.length - 1 ? "." : ""}
                            <br />
                          ...
                          {results.rows.map((row: any, rowIndex: number) => (
                            ...
                            >
                              {results.columns.map((column: string) => (
                                <TableCell key={column} className="font-mono text-sm text-[var(--foreground)] dark:text-[var(--card-foreground)] bg-[var(--card)] dark:bg-[var(--card)]">
                                  {row[column] === null ? (
